...
- **Jobs**: `list_jobs`, `get_job_output`, `get_job_status`, `spawn_worker`, `cancel_job`, `retry_job`
- **MCP**: `list_mcps`, `register_mcp`, `delete_mcp`, `test_mcp`
- **Feedback**: `submit_feedback`, `list_feedback`, `approve_evolution`, `reject_evolution`
- **Logs**: `get_logs`
- **Schedules**: `list_schedules`, `create_schedule`, `update_schedule`, `toggle_schedule`, `delete_schedule`
...
get_job_status({ jobId: string }) → JobStatus
// SELECT status, created_at, started_at, completed_at FROM ouro_jobs WHERE id=$1
...
retry_job({ jobId: string }) → Job
// Clones a failed/cancelled job as a new pending job with a fresh ID
// Uses job.instructions (or job.description fallback) as the new task instructions
// Only valid for jobs with status 'failed' or 'cancelled'
```
...
// SELECT * FROM ouro_schedules ORDER BY created_at DESC

create_schedule({ name, cron_expr, backend, target, instructions }) → Schedule
// INSERT INTO ouro_schedules (...) RETURNING *
// Validates cron_expr via croner. Computes initial next_run_at.

update_schedule({ id, name?, cron_expr?, backend?, target?, instructions? }) → { updated: boolean }
// Partial UPDATE — only provided fields are changed; omitted fields keep current values
// If cron_expr changes, recomputes next_run_at

toggle_schedule({ id: string }) → { enabled: boolean }
// Flip ouro_schedules.enabled. Returns new state.

delete_schedule({ id: string }) → { deleted: boolean }
// DELETE FROM ouro_schedules WHERE id=$1
```
